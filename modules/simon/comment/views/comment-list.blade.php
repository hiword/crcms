
<?php 
function comment_tree_view($model)
{
	$time = format_date($model->created_at);
	
	$string = <<<string
	<div class="media">
  <div class="media-left">
    <a href="#">
    <!--
      <img class="media-object" src="..." alt="...">
      -->
      <img class="media-object" alt="64x64" data-src="holder.js/64x64" style="width: 64px; height: 64px;" src="data:image/svg+xml;base64," data-holder-rendered="true">
    </a>
  </div>
  <div class="media-body comment-body">
  	<!--
    <h4 class="media-heading">Media heading</h4>
    -->
    <p class="clearfix"><a href="###" class="user-name pull-left">Name</a><span class="pull-right time">{$time}</span></p>
    <p class="content">{$model->content}</p>
    <p class="next-action"><a href="###"><i class="glyphicon glyphicon-edit"></i>&nbsp;回复</a><a href="###"><i class="glyphicon glyphicon-thumbs-up"></i>&nbsp;顶</a></p>
  </div>
</div>
string;
  return $string;
}
    
function comment_tree($models,$id = 0)
{
	foreach ($models as $model)
	{
// 		!isset($model['append']) && $model['append'] = [];
// 		if ($model['reply_id'] <> 0)
// 		{
// 			$model['append'] = comment_tree($model);
// 		}
		print comment_tree_view($model);
	}
}

?>
<style>
.comment-body a {text-decoration:none;}
.comment-body p {margin-bottom:5px;}
.comment-body .next-action a {margin-right:15px;color:#999999;font-size:12px; }
.comment-body .time {color:#999999}
.comment-body .content {color:#666666}
.comment-body .user-name {color:#428bca!important;}
</style>
<div class="comment-list"><?php comment_tree($models);?></div>