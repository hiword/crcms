...
// 	protected $coverFile = false;
	...
	{
		$path && $this->path = $path;
		//创建目录 realpath使用，否则会报错
		!is_dir($this->path) && $this->mkDir($this->path);
		//linux风格 Love Linux
	...
	protected function checkUpload(array $upload)
	{
		...
		//验证文件大小
		$this->checkFileSize($upload);
	
		//验证扩展名
		$this->checkFileExtension($upload);
	
		//验证mime
		$this->checkFileMime($upload);
	 ...
	 * @author simon
	 */
	protected function checkFileMime(array $upload)
	{
		if (!$this->checkMime)
		{
			return true;
		...
		$extension = $this->file->getExtension($upload['name']);
		//jpg文件mime特例
		...
		{
			throw new TypeErrorException($upload['name'],'mime');
		}
		return true;
	}
	 ...
	 * @author simon
	 */
	protected function checkFileExtension(array $upload)
	{
		...
		{
			throw new TypeErrorException($upload['name'],'extension');
		}
		return true;
	}
	 ...
	 * @author simon
	 */
	protected function checkUploadSelf(array $upload)
	{
		if ($upload['error'] !== UPLOAD_ERR_OK)
		{
			throw new UploadException($upload['name'],$upload['error']);
		}
		return true;
	}
	 ...
	 * @author simon
	 */
	protected function checkFileSize(array $upload)
	{
		//验证文件大小
		if ($this->filesize < $this->file->getSize())
		{
			throw new SizeException($upload['name']);
		}
		return true;
	}
...
// 	}
	 ...
	 * @author simon
	 */
	protected function getFilePath($dir,$name)
	{
